...
// Créer le graphique une seule fois lors du chargement de la page
...
function submitForm() { 
    ...
    if(selectedYear)
    {
        ...
        .then(data => {
            // Remodeler les données pour utiliser les noms des mois
            const moisMap = {
                1: 'Janvier',
                2: 'Février',
                3: 'Mars',
        ...
        .then(data => {
            ...
            // Affichezr les données dans la console
        ...
        .then(response => response.json())
        .then(data => {
            // Afficher la réponse dans le div "resultatRequete"
            const resultatRequete = document.getElementById('nombres2');
            const average = data[0].average_victim_count;
            // Utiliser l'option 'fr-FR' pour la France
...
function updateConnectionStatus() {
  // Effectuer une requête AJAX au point de terminaison '/connection-status' pour obtenir l'état de la connexion
...
// Mettre à jour l'état de connexion toutes les quelques secondes
